        ...
        try:
            ...
            try:
                ...
                ToolCallResultWorkflowEvent(
                    ...
                    content=json.dumps(tool_outputs, default=str),
                    role="tool"
                )
            )
            ...
            try:
                if tool.metadata.return_direct:
                    try:
                        cnt = tool_output.raw_output['response']
                    except Exception as e:
                        print(f"DEBUG: Error getting tool output content: {e}")
                        print(type(tool_output.raw_output))
                        print(tool_output.raw_output)
                        cnt = str(tool_output.content)
