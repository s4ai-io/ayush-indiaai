...
2.  **Multilingual Support**: The doctor may speak in mixed English/Indian languages. Translate clinical terms to English.
...
4.  **Proactive Filling**: Update the form IMMEDIATELY with ANY available information.
    ...
    -   Call `fill_clinical_assessment` after EVERY user input that contains relevant data.
5.  **Auto-Generate**: Once key fields (disease, doshas, prakriti) are filled, suggest generating the AI plan.

STRICT DATA FORMATTING:
Use `fill_clinical_assessment` with these EXACT fields:
...
async def fill_clinical_assessment(
    ctx: Context,
    ...
    """
    Updates the clinical assessment form with the provided information.
    """
    return "Clinical assessment form updated successfully."
...
treatment_agent_router = get_ag_ui_workflow_router(
    ...
    backend_tools=[],
    frontend_tools=[fill_clinical_assessment, generate_treatment_plan],
